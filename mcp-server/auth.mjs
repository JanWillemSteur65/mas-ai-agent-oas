...
export function authMiddleware(secret) {
  return function(req, _res, next) {
    // Service-to-service auth (no browser cookies)
    const internal = req.headers["x-internal-token"];
    const expected = process.env.MCP_INTERNAL_TOKEN || process.env.INTERNAL_TOKEN;
    if (expected && internal && internal === expected) {
      req.user = { username: "internal", role: "admin" };
      return next();
    }
