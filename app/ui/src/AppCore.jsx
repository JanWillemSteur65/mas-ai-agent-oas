...
async function apiAgentChat(payload) {
  const r = await fetch('/api/agent/chat', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
...
async function apiAnalyzeLast(payload) {
  const r = await fetch('/api/agent/analyze-last', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
	if(!r.ok) {
	  const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiFollowup(payload) {
  const r = await fetch('/api/agent/followup', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
  if(!r.ok) {
    const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiValueListTenants(payload) {
  const r = await fetch('/api/agent/value-list/tenants', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
	if(!r.ok) {
	  const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiTenantsInfo(payload) {
  const r = await fetch('/api/agent/tenants-info', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
  if(!r.ok) {
    const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiTenantsRaw(payload) {
  const r = await fetch('/api/agent/tenants-raw', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
  if(!r.ok) {
    const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiConcepts(payload) {
  const r = await fetch('/api/agent/concepts', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
  if(!r.ok) {
    const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiValueListAssets(payload) {
  const r = await fetch('/api/agent/value-list/assets', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
	if(!r.ok) {
	  const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiCreateRecord(payload) {
  const r = await fetch('/api/agent/create-record', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
	if(!r.ok) {
	  const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiAgentQueryOS(payload) {
  const r = await fetch('/api/agent/query-os', { method:'POST', headers:{'content-type':'application/json'}, credentials:'include', body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
	if(!r.ok) {
	  const detail = (j && (j.detail || j.error)) ? (j.detail || j.error) : (raw || `HTTP ${r.status}`)
...
async function apiMaximoIntentQuery(payload) {
  const r = await fetch('/api/maximo/intent-query', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify(payload) })
  const raw = await r.text()
  let j=null; try{ j=JSON.parse(raw) }catch{}
...
function AssistiveChips({ catalog, onPick, hideTitle = false }) {
  const cat = catalog || {}

  // Concepts v2 preferred: ui.chips (grouped suggestions)
  const v2chips = Array.isArray(cat?.ui?.chips) ? cat.ui.chips : []
  if (v2chips.length) {
    // Render compact: a single horizontal/wrapping row, with group context only as tooltip.
    const items = v2chips
      .map((ch) => ({
        group: String(ch?.group || 'Suggestions'),
        label: String(ch?.text || ''),
        text: String(ch?.text || ''),
        intentHint: ch?.intentHint || null,
      }))
      .filter((x) => x.label)
    return (
      <div className="mx-chips" style={{ marginTop: 10 }}>
        {!hideTitle ? <div className="mx-chips-title">Suggestions</div> : null}
        <div className="mx-chips-row mx-chips-row--compact">
          {items.slice(0, 24).map((it, idx) => (
            <Button
              ...
              kind="tertiary"
              className="mx-chip"
              onClick={() => onPick?.({ text: it.text, intentHint: it.intentHint })}
            >
              {it.label}
            </Button>
          ))}
  ...
  // Back-compat: Concepts v1 (or v2.legacy)
  const legacy = cat?.legacy || cat
  const entities = Array.isArray(legacy?.entities) ? legacy.entities : []
  const quick = Array.isArray(legacy?.quickPhrases) ? legacy.quickPhrases : []
  const statusPhrases = Array.isArray(legacy?.statusPhrases) ? legacy.statusPhrases : []

  const groups = []
  if (entities.length) groups.push({ title: 'Entities', items: entities.map((e) => ({ label: String(e.label || e.id), text: String(e.example || e.label || e.id) })) })
  if (statusPhrases.length) groups.push({ title: 'Status', items: statusPhrases.map((x) => ({ label: String(x.label || x.text), text: String(x.text || x.label) })) })
  if (quick.length) groups.push({ title: 'Common', items: quick.map((x) => ({ label: String(x.label || x.text), text: String(x.text || x.label) })) })
    ...
    <div className="mx-chips" style={{ marginTop: 10 }}>
      ...
      {groups.map((g) => (
        <div key={g.title} style={{ marginBottom: 8 }}>
          <div style={{ opacity: 0.75, fontSize: 12, marginBottom: 6 }}>{g.title}</div>
          <div className="mx-chips-row mx-chips-row--compact">
            {g.items.slice(0, 12).map((it, idx) => (
              <Button key={`${g.title}-${idx}`} size="sm" kind="tertiary" className="mx-chip" onClick={() => onPick?.({ text: it.text, intentHint: it.intentHint })}>
                {it.label}
              </Button>
            ))}
          </div>
...
function PromptChips({ onPick, prompts, hasAnalysis, hideTitle = false }) {
  return (
    <div className="mx-chips">
      {!hideTitle ? <div className="mx-chips-title">Predefined prompts</div> : null}
...
function SettingsBody({ local, setLocal, onSave, darkMode, setDarkMode, showSaveButton=true, role, collapseAll=false, saveButtonText="Save settings" }) {
  ...
  const modelSig = (() => {
    ...
    // Support both legacy and current watsonx settings keys so changing either triggers a refresh.
...
const [tenantCatalogRawError, setTenantCatalogRawError] = useState('')
  ...
  const applyTenantSelection = (tenantId) => {
    ...
    if (!tid) return
    const info = (tenantCatalog || []).find((t) => String(t?.id || t?.tenant || t?.tenantId || t?.name || '').trim() === tid) || null
    const raw = (tenantCatalogRaw || []).find((t) => String(t?.id || t?.tenant || t?.tenantId || t?.name || '').trim() === tid) || null

    // Keep the selection consistent everywhere: tenant id always changes, and when we have
    // tenant metadata/secrets we also auto-fill baseUrl/apiKey/defaultSite.
    const next = (prev) => ({
      ...prev,
      maximo: {
        ...(prev?.maximo || {}),
    ...
    // Apply locally (Settings UI) and also push immediately to the app settings state so the
    // Chat page tenant dropdown + queries stay in sync.
    setLocal((p) => {
      const n = next(p)
      try { onSave?.(n) } catch {}
      return n
	...
	// NOTE: Defined *before* JSX uses it to avoid ReferenceErrors in production builds.
	const refreshTenantCatalog = async () => {
	  ...
	  // Raw (may include apiKey). The backend can choose to redact/deny; the UI should
	  // treat failures as non-fatal (it only affects the auto-fill UX).
	  try {
  ...
  // Query synonyms are managed in the MCP Server UI (NLQ Rules).
	...
	// Keep the record-link template editor in sync with saved settings.
	// Only auto-populate when the editor is empty to avoid clobbering in-progress edits.
	useEffect(() => {
		try {
			if (String(linkTplText || '').trim()) return
			const obj = local?.maximoUi?.recordLinkTemplates || {}
			const txt = JSON.stringify(obj, null, 2)
			setLinkTplText(txt === '{}' ? '' : txt)
		} catch {}
		// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="mx-page">
      ...
      <details className="mx-card" open={detailsOpen}>
  ...
  <Button size="sm" kind="ghost" onClick={refreshTenantCatalog} disabled={!mcpUrl || tenantCatalogBusy || !isAdmin}>
    Refresh tenants from MCP
	                  ...
	                  <TableRow key={row.id} {...getRowProps({ row })}>
	                    {row.cells.map((cell) => {
	                      ...
	                      if (headerKey === 'actions') {
	                        ...
	                        return (
	                          <TableCell key={cell.id} className="mx-td-actions">
	      ...
	      <Modal
	        ...
	        className="mx-settings-submodal"
	      >
	        <div className="mx-form">
...
function SettingsDialog({ open, onClose, settings, setSettings, darkMode, setDarkMode, role }) {
  ...
  // Save helper. Accepts an optional override so inner controls (like tenant pickers)
  // can apply immediately without waiting for the modal submit.
  const save = (override) => {
    const next = override && typeof override === 'object' ? override : local
    setSettings(next)
...
function SettingsPage({ settings, setSettings, darkMode, setDarkMode, role }) {
  ...
  const save = (override) => {
    ...
    setSettings(next)
    persistSettings(next)
    // Always attempt server persistence; non-admin writes are safely limited server-side.
    apiSaveSettings(next).catch(() => {})
    // Match Settings dialog behavior (Save & Close): return to chat after saving.
    nav('/chat')
  }

  return (
    <div className="mx-settings-page">
      <SettingsBody
        local={local}
        setLocal={setLocal}
        onSave={save}
...
function ChatPage({ settings, setSettings, mode, setMode }) {
  ...
  const [pendingIntentHint, setPendingIntentHint] = useState(null)
  ...
  // Assistive UI (concept-driven suggestions). Best-effort; never blocks chat.
  const [conceptCatalog, setConceptCatalog] = useState(null)
  const [conceptError, setConceptError] = useState('')
  const activeTenantId = useMemo(() => String(settings?.maximo?.defaultTenant || 'default').trim(), [settings?.maximo?.defaultTenant])

  useEffect(() => {
    let alive = true
    ;(async () => {
      const mcpUrl = String(settings?.mcp?.url || '').trim()
      if (!mcpUrl) { if (alive) { setConceptCatalog(null); setConceptError('') } ; return }
      try {
        const j = await apiConcepts({ settings, tenant: activeTenantId })
        if (alive) { setConceptCatalog(j); setConceptError('') }
      } catch (e) {
        if (alive) { setConceptCatalog(null); setConceptError(String(e?.message || e)) }
      }
    })()
    return () => { alive = false }
  }, [settings?.mcp?.url, activeTenantId])
  ...
  const send = async (forced, action = null) => {
    ...
    const intentHint = (action && action.intentHint) ? action.intentHint : pendingIntentHint
...
- When users ask for open work orders, filter out status CLOSE and COMP.`.trim()
    ...
    setInput('')
    setPendingIntentHint(null)
    try {
      if (effectiveMode === 'maximo') {
        const resp = intentHint ? await apiMaximoIntentQuery({ text: textForMaximo, intentHint, settings }) : await apiMaximoQuery({ text: textForMaximo, settings })
	  ...
	  const runQueryPresetById = async (presetId) => {
	    const presets = Array.isArray(settings?.maximo?.queryPresets) ? settings.maximo.queryPresets : []
  ...
  const pickPrompt = async (p) => {
    // Predefined prompts must work in both modes. Do not force mode switches.
    if (p?.action === "create_wo") { return openCreate("wo") }
    if (p?.action === "create_sr") { return openCreate("sr") }
    if (p?.action === "query_preset") {
      const pid = String(p?.presetId || '').trim()
      const preset = (settings?.maximo?.queryPresets || []).find((x) => String(x?.id || '').trim() === pid) || null
      if (!preset) {
        setMessages((m) => [...m, { role:'assistant', source: (mode === 'ai' ? 'ai' : 'maximo'), text: `Error: preset "${pid}" not found in Settings → Query presets.` }])
        return
      }

      // In Maximo mode, run the preset directly as a tool call (no LLM required).
      if (mode === 'maximo') {
        const userText = `Run query preset: ${preset.label || preset.id}`
        appendPromptHistory(userText)
        setMessages((m) => [...m, { role:'user', text: userText }])
        return runQueryPresetById(preset.id)
      }

      // In AI Agent mode, keep the previous behavior (the agent can still decide to call tools).
  ...
  return (
    <div className="mx-chat-page">
      ...
      <div className="mx-chat-card">
        ...
        {/* Suggestions + Predefined prompts sit directly under the prompt field.
            Both are collapsible; suggestions start collapsed by default. */}
        <details className="mx-chip-section" style={{ marginTop: 10 }}>
          <summary className="mx-chips-title" style={{ cursor: 'pointer', listStyle: 'none' }}>Suggestions</summary>
          <div style={{ marginTop: 8 }}>
            <AssistiveChips
              hideTitle
              catalog={conceptCatalog}
              onPick={({ text: t, intentHint }) => {
                const ins = String(t || '').trim()
                if (!ins) return
                setInput((prev) => {
                  const p = String(prev || '')
                  return (p && !p.endsWith(' ') ? p + ' ' : p) + ins
                })
                setPendingIntentHint(intentHint || null)
              }}
            />
          </div>
        </details>

        <details className="mx-chip-section" style={{ marginTop: 10 }} open>
          <summary className="mx-chips-title" style={{ cursor: 'pointer', listStyle: 'none' }}>Predefined prompts</summary>
          <div style={{ marginTop: 8 }}>
            <PromptChips hideTitle onPick={pickPrompt} prompts={prompts} hasAnalysis={hasAnalysis} />
          </div>
        </details>
      </div>

      {/* Legacy placement removed (now embedded under the prompt field). */}
  ...
  return (
    <Layout darkMode={darkMode} setDarkMode={setDarkMode} navExpanded={navExpanded} setNavExpanded={setNavExpanded}>
      <Routes>
        ...
        {/* Settings are available via the header settings icon dialog. Keep this route only as a redirect for old bookmarks. */}
